...
import type { PublicOwner } from '@spaceorder/db';
...
export class OwnerController {
  ...
  @ApiResponse(ownerDocs.unauthorizedResponse)
  async getList(): Promise<PublicOwner[]> {
  ...
  @Get(':ownerId')
  @UseGuards(ZodValidation({ params: ownerIdParamsSchema }), OwnerPermission)
  ...
  async getUnique(@Param('ownerId') ownerId: string): Promise<PublicOwnerDto> {
    const findOwner = await this.ownerService.getOwnerUnique({
      where: { publicId: ownerId },
    });

    return new PublicOwnerDto(findOwner);
  }
